...
clear all;
    ...
    [w1_opt, w2_opt, w3_opt, w4_opt, f_val] = optimize_weights();
...
%%
    ...
    WGMVP = numerator / denominator;

    % ********** WEUP **********
    w1 = (1 / L) * (C * miu);

    % 分开计算分子和分母
    numerator1 = C * one * (one' * C );  % 分子部分
    denominator1 = one' * C * one;          % 分母部分
    w2 = (1 / L) * (numerator1 / denominator1)*miu;
    WEUP1 = WGMVP + w1 - w2;
%%
   ...
   save('Temp\J_0.5\WEUP_pp300.mat','WEUP');
